...
@Provider
public class ProductExceptionMapper implements ExceptionMapper<javax.validation.ValidationException> {
    @Override
    public Response toResponse(javax.validation.ValidationException e) {
        ...
        return Response.status(Response.Status.BAD_REQUEST).entity(new JsonError("Validation",
                stringBuilder.toString())).type(MediaType.APPLICATION_JSON).build();
